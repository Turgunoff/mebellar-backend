...
func (e *EskizService) Login() error {
	...
	if err := writer.WriteField("email", e.email); err != nil {
		return fmt.Errorf("email field yaratishda xatolik: %v", err)
	}
	if err := writer.WriteField("password", e.password); err != nil {
		return fmt.Errorf("password field yaratishda xatolik: %v", err)
	}
	writer.Close()
